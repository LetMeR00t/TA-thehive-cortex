...
def process_event(helper, *args, **kwargs):
    ...
    for thehive, configuration, defaults, logger_file, instance_id in instances:
        ...
        logger_file.info(
            id="56", message="Configuration is ready. Running the function..."
        )
        logger_file.debug(
            id="57",
...
def run_function(helper, thehive: TheHive4Splunk, alert_args):
    ...
    for row in helper.get_events():
        ...
        thehive.logger_file.debug(
            id="65", message="Row before pre-processing: " + str(row)
        )
    ...
    thehive.logger_file.debug(
        id="70",
        message='Run function "{}" with the events: '.format(alert_args["name"]),
    )
    ...
    if "result" in response:
        thehive.logger_file.info(
            id="75", message="Function was run on the events: {}".format(response)
        )
    else:
        thehive.logger_file.error(
            id="80",
            message="Function didn't run correctly on the events: {}".format(response),
        )
