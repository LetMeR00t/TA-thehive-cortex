    ...
    def __recover(exception):
        ...
        if isinstance(exception, requests.exceptions.HTTPError):
            if exception.response.status_code == 404:
                ...
                raise InvalidInputError("Invalid input exception")
        elif isinstance(exception, requests.exceptions.ConnectionError):
            raise ServiceUnavailableError("Cortex service is unavailable")
        elif isinstance(exception, requests.exceptions.RequestException):
            raise ServerError("Cortex request exception")
        else:
            raise CortexError("Unexpected exception")
